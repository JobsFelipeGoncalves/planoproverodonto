...
<html lang="pt-br">
  <head>    
    ...
    <title>Central de Ajuda | Gerencie mais</title>
  </head>
  <body>
        ...
        <div class="container pt-3 pb-4">
            <div class="row">
                <div class="col-md fonte-20">
                    ...
                    <span class = "cor-preto-b">Central de Ajuda</span>
                </div>
...
?>
        ...
        <div class="row mb-2 mt-2 pt-3 pb-2">
            <div class="col-md-2">
                <a href="centralDeAjudaAddAssunto.php?Retorno=<?= $URL_ATUAL ?>&Acao=Novo&Status=Criando&Rotativo=<?php echo $idRotativo = rand(0000000000,9999999999); ?>" class = "botao botao-vermelho-f" title = "Clique para criar um novo conteúdo"> + Novo assunto</a>
            </div>
            <div class="col-md-8"><!-- nulo --></div>
            <div class="col-md-2">
                <a href="centralDeAjudaLixo.php?Acao=Listar&Status=TodosDaLixeira" class = "botao botao-cinza" title = "Clique parar ver o que tem na lixeira"> <span class="material-icons-round icones">delete_sweep</span>Ver a lixeira</a>
            </div>
        </div>
                            ...
                            <div class="col-md-1 text-center">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuLink">
                                        <li><a class="dropdown-item" href="centralDeAjudaPrevia.php?Acao=Visualizar&Rotativo=<?= $registo['id'] ?>&Status=Previa"><span class="material-icons-round icones">description</span>Ver detalhes</a></li>
                                        <li><a class="dropdown-item" href="centralDeAjudaEdicaoAssunto.php?Retorno=<?= $URL_ATUAL ?>&novaAcao=Editar&Rotativo=<?= $registo['id'] ?>&Status=Editando"><span class="material-icons-round icones">create</span>Editar</a></li>
                                        <?php if($registo['status'] == "Oculto") { $iconeVisivel = "remove_red_eye"; $textoVisivel = "Deixar visivel"; $textoLink = "Desocultar"; ?>
                                        <li><a class="dropdown-item" href = "centralDeAjuda.php?Retorno=<?= $URL_ATUAL ?>&novaAcao=<?= $textoLink ?>&Rotativo=<?= $registo['id'] ?>&Status=<?= $textoLink ?>"><span class="material-icons-round icones"><?= $iconeVisivel ?></span> <?= $textoVisivel ?></a></li>
                                     ...
                                     <div class="modal fade" id="modal_ocultar_<?= $registo['id'] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                                ...
                                                <div class="modal-body text-center pl-5 pr-5">
                                                        ...
                                                        <p class = "p-2">Ao clicar em <strong>"Sim, ocultar"</strong>, o "<?= $registo['assunto'] ?>" não será visível para os usuários.</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="botao botao-nulo" data-bs-dismiss="modal">Cancelar</button>
                                                    <a href = "centralDeAjuda.php?Retorno=<?= $URL_ATUAL ?>&novaAcao=<?= $textoLink ?>&Rotativo=<?= $registo['id'] ?>&Status=<?= $textoLink ?>" class="botao botao-vermelho-f">Sim, ocultar</a>
                                                </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="modal fade" id="modal_lixo_<?= $registo['id'] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                                ...
                                                <div class="modal-body text-center pl-5 pr-5">
                                                        ...
                                                        <p class = "p-2">Você pode recuperar o "<?= $registo['assunto'] ?>" lá na lixeira depois, ok?</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="botao botao-nulo" data-bs-dismiss="modal">Cancelar</button>
                                                    <a href = "centralDeAjuda.php?Retorno=<?= $URL_ATUAL ?>&novaAcao=Lixeira&Rotativo=<?= $registo['id'] ?>&Status=MoverLixeira" class="botao botao-vermelho-f">Mover para o lixo</a>
                                                </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <?php
        ...
        if (isset($_GET['novaAcao'])) {
            ...
            if ($acaoRecebida == "Ocultar") {
                ...
                $selecionaAcao = "UPDATE ajuda_assunto SET status = 'Oculto' WHERE id = '$acoRotativo'";
            ...
            if ($acaoRecebida == "Lixeira") {
                 ...
                 // echo "Ok! Vamos ocultar conteúdo";
                 $selecionaAcao = "UPDATE ajuda_assunto SET status = 'Lixeira' WHERE id = '$acoRotativo'";
                 $consultaAcao = $conexao -> prepare($selecionaAcao);
                 $consultaAcao -> execute();
